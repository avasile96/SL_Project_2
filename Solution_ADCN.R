...
# correlation
# train_cor = cor(db_train[-c(1,568)])
...
numNegative <-length(which(db_train$Labels==levels(db_train$Labels)[2]))
...
# newSamples <- oversample(db_train[2:ncol(db_train)] ,method = "SMOTE",classAttr = 'Labels', ratio = imb_idx*3) # ADCN
# newSamples <- oversample(db_train[2:ncol(db_train)] ,method = "SMOTE",classAttr = 'Labels', ratio = imb_idx*5) # ADMCI
...
acc_knn <- performance(pred_knn,"acc")
spec_knn <- performance(pred_knn,"spec")
...
svmRadial.fit <- train(Labels ~ .,
                ...
                tuneGrid = expand.grid(sigma = seq(from = 0.001, to = 0.002, by = 0.0002),
                                       C = seq(from = 1, to = 2, by = 0.10)),
                trControl = train.control)
